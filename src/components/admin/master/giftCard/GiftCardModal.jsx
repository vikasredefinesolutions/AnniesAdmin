import React, { useState, useCallback, useEffect } from "react";
import { Formik, Form as FormikForm } from "formik";
import { useSelector, useDispatch } from "react-redux";
import * as Yup from "yup";

import {
  CurrencySymbolByCode,
  RecStatusValuebyName,
  anniesAnnualData,
} from "global/Enum";
import { ValidationMsgs } from "global/ValidationMessages";

import CustomerGiftCardServices from "services/admin/customer/CustomerGiftCardServices";
import { TitleNameHelper, serverError } from "services/common/helper/Helper";
import { GenerateRandomCodeByLength } from "services/common/helper/Helper";
import StoreService from "services/admin/store/StoreService";

import { setAddLoading } from "redux/globalLoader/GlobalLoaderAction";
import { setAlertMessage } from "redux/alertMessage/AlertMessageReducers";

import ToggleButton from "components/common/formComponent/ToggleButton";
import DatePicker from "components/common/formComponent/DatePicker";
import Messages from "components/common/alerts/messages/Index";
import Input from "components/common/formComponent/Input";
import InputNumber from "components/common/formComponent/InputNumber";

const CustomerAddUpdateModal = ({
  handleShowModal,
  getCustomerGiftCardData,
  editId,
}) => {
  const dispatch = useDispatch();

  const GlobalLoading = useSelector(
    (store) => store?.GlobalLoaderReducer?.toLoad
  );
  const company = useSelector((store) => store?.CompanyConfiguration);
  const permission = useSelector((store) => store.permission);
  const location = useSelector((store) => store?.location);
  const user = useSelector((store) => store?.user);

  const [data, setData] = useState([]);

  const isAddMode = editId ? false : true;

  const createCustomerGiftCard = (fields, resetForm) => {
    dispatch(setAddLoading(true));
    CustomerGiftCardServices.createCustomerGiftCard({
      giftCardModel: { ...fields, ...location },
    })
      .then((response) => {
        if (response.data.success) {
          dispatch(
            setAlertMessage({
              type: "success",
              message: ValidationMsgs.customerGiftCard.customerGiftCardCreated,
            })
          );
          resetForm({});
          handleShowModal((prev) => !prev);
          getCustomerGiftCardData();
        } else {
          dispatch(
            setAlertMessage({
              type: "danger",
              message: serverError(response),
            })
          );
        }
        dispatch(setAddLoading(false));
      })
      .catch((error) => {
        dispatch(
          setAlertMessage({
            type: "danger",
            message: ValidationMsgs.customerGiftCard.customerGiftCardNotCreated,
          })
        );
        dispatch(setAddLoading(false));
      });
  };

  const updateCustomerGiftCard = (fields, resetForm) => {
    dispatch(setAddLoading(true));
    CustomerGiftCardServices.updateCustomerGiftCard({
      giftCardModel: { ...fields, ...location },
    })
      .then((response) => {
        if (response.data.success) {
          dispatch(
            setAlertMessage({
              type: "success",
              message: ValidationMsgs.customerGiftCard.customerGiftCardUpdated,
            })
          );
          handleShowModal((prev) => !prev);
          resetForm({});
          getCustomerGiftCardData();
        } else {
          dispatch(
            setAlertMessage({
              type: "danger",
              message: serverError(response),
            })
          );
        }
        dispatch(setAddLoading(false));
      })
      .catch((error) => {
        dispatch(
          setAlertMessage({
            type: "danger",
            message: ValidationMsgs.customerGiftCard.customerGiftCardNotUpdated,
          })
        );
        dispatch(setAddLoading(false));
      });
  };

  const onSubmit = (fields, { resetForm }) => {
    if (editId) {
      updateCustomerGiftCard(fields, resetForm);
    } else {
      createCustomerGiftCard(fields, resetForm);
    }
  };

  const initialValues = {
    id: data?.id || 0,
    storeID: data?.storeID || parseInt(anniesAnnualData.storeId),
    sku: data?.sku || "",
    issueDate: data?.issueDate || "",
    recipientName: data?.recipientName || "",
    emailTo: data?.emailTo || "",
    serialNumber: data?.serialNumber || GenerateRandomCodeByLength(16),
    orderNumber: data?.orderNumber || 0,
    initialAmount: data?.initialAmount || 0,
    balance: data?.balance || 0,
    status: data?.status || false,
    customerID: data?.customerID || 0,
    rowVersion: data?.rowVersion || "",
    location: data?.location || "",
    ipAddress: location?.ipAddress || "",
    macAddress: location?.macAddress || "",
    recStatus: RecStatusValuebyName.Active,
    ismanual: data?.ismanual || false,
    expirationDate: data?.expirationDate || "",
    emailFrom: data?.emailFrom || "",
    emailMessage: data?.emailMessage || "",
    rmaNumber: data?.rmaNumber || 0,
    giftCardType: data?.giftCardType || 0,
  };

  const validationSchema = Yup.object({
    storeID: Yup.string().trim(),
    recipientName: Yup.string().trim(),
  });

  useEffect(() => {
    if (editId) {
      dispatch(setAddLoading(true));
      CustomerGiftCardServices.getByIdCustomerGiftCard(editId)
        .then((res) => {
          var response = res.data;
          if (response.success) {
            setData({ ...response.data });
          }
          dispatch(setAddLoading(false));
        })
        .catch((err) => {
          dispatch(setAddLoading(false));
        });
    }
  }, [editId]);

  return (
    <>
      <div
        id="paymentModal"
        className="overflow-y-auto overflow-x-hidden z-30 fixed right-0 left-0 top-4 justify-center items-center h-modal md:h-full md:inset-0"
      >
        <div className="w-full h-full bg-black bg-opacity-50 flex items-center justify-center">
          <div className="relative px-4 w-full max-w-2xl h-full md:h-auto">
            <div className="relative bg-white rounded-lg shadow max-h-screen overflow-y-auto">
              <div className="flex justify-between items-start p-5 rounded-t border-b sticky top-0 left-0 bg-white">
                <h3 className="text-xl font-semibold text-gray-900 lg:text-2xl">
                  {isAddMode === true ? "Add " : "Edit "}
                  {TitleNameHelper({ defaultTitleName: `Customer Gift Card` })}
                </h3>
                <button
                  type="button"
                  className="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center"
                  data-modal-toggle="actionModal"
                  onClick={handleShowModal}
                >
                  <svg
                    className="w-5 h-5"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      fillRule="evenodd"
                      d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
                      clipRule="evenodd"
                    />
                  </svg>
                </button>
              </div>
              <Formik
                enableReinitialize={true}
                initialValues={initialValues}
                validationSchema={validationSchema}
                onSubmit={onSubmit}
              >
                {({ setFieldValue, values }) => {
                  return (
                    <FormikForm>
                      <Messages />
                      <div className="p-6">
                        {!isAddMode && (
                          <div className="w-full mb-4 last:mb-0">
                            <label
                              className="block uppercase tracking-wide text-gray-500 text-xs font-bold mb-2"
                              htmlFor="grid-first-name"
                            >
                              GIFT CARD NUMBER
                              <span className="text-rose-500 text-2xl leading-none">
                                *
                              </span>
                            </label>
                            <div className="flex justify-center items-center">
                              <Input
                                name="serialNumber"
                                disabled={true}
                                placeholder={`16 Digit Auto Generated`}
                                className={"bg-slate-300"}
                              />
                              {/* {isAddMode && (
                              <button
                                type="button"
                                onClick={() =>
                                  setFieldValue(
                                    "serialNumber",
                                    GenerateRandomCodeByLength(16)
                                  )
                                }
                                className={`flex justify-center btn px-6 ml-2 bg-slate-300 border-none`}
                                disabled={
                                  values.customerID === 0 ? false : true
                                }
                              >
                                Re Generate
                              </button>
                            )} */}
                            </div>
                          </div>
                        )}
                        <div className="w-full mb-4 last:mb-0">
                          <label
                            className="block uppercase tracking-wide text-gray-500 text-xs font-bold mb-2"
                            htmlFor="grid-first-name"
                          >
                            SKU
                            {/* <span className="text-rose-500 text-2xl leading-none">
                              *
                            </span> */}
                          </label>
                          <Input
                            name="sku"
                            disabled={values.customerID === 0 ? false : true}
                          />
                        </div>

                        <div className="w-full mb-4 last:mb-0">
                          <label
                            className="block uppercase tracking-wide text-gray-500 text-xs font-bold mb-2"
                            htmlFor="grid-first-name"
                          >
                            {`AVAILABLE BALANCE (${CurrencySymbolByCode.USD})`}
                            <span className="text-rose-500 text-2xl leading-none"></span>
                          </label>
                          <InputNumber
                            name={"balance"}
                            allowNegative={true}
                            placeholder="0.00"
                            value={values.balance}
                            displayError={true}
                            onChange={(e) => {
                              setFieldValue("balance", e.target.value);
                            }}
                            disabled={values.customerID === 0 ? false : true}
                          />
                        </div>

                        <div className="w-full mb-4 last:mb-0">
                          <label
                            className="block uppercase tracking-wide text-gray-500 text-xs font-bold mb-2"
                            htmlFor="grid-first-name"
                          >
                            {`ISSUE BALANCE (${CurrencySymbolByCode.USD})`}
                            <span className="text-rose-500 text-2xl leading-none"></span>
                          </label>
                          <InputNumber
                            name={"initialAmount"}
                            allowNegative={true}
                            placeholder="0.00"
                            value={values.initialAmount}
                            displayError={true}
                            onChange={(e) => {
                              setFieldValue("initialAmount", e.target.value);
                            }}
                            disabled={values.customerID === 0 ? false : true}
                          />
                        </div>

                        <div className="w-full mb-4 last:mb-0">
                          <label
                            className="block uppercase tracking-wide text-gray-500 text-xs font-bold mb-2"
                            htmlFor="grid-first-name"
                          >
                            Expiry Date
                            <span className="text-rose-500 text-2xl leading-none"></span>
                          </label>
                          <DatePicker
                            className={"mr-2"}
                            name={"issueDate"}
                            defaultValue={values.issueDate}
                            placeholder="Select Date"
                            disabled={values.customerID === 0 ? false : true}
                          />
                        </div>
                        <div className="w-full mb-4 last:mb-0">
                          <label
                            className="block uppercase tracking-wide text-gray-500 text-xs font-bold mb-2"
                            htmlFor="grid-first-name"
                          >
                            Is Active
                            <span className="text-rose-500 text-2xl leading-none"></span>
                          </label>
                          <ToggleButton
                            name="recStatus"
                            id="recStatus"
                            size="m"
                            on="Active"
                            off="Inactive"
                            defaultValue={values?.status}
                            onChange={(e) => {
                              setFieldValue("status", e.target.checked);
                            }}
                            disabled={values.customerID === 0 ? false : true}
                          />
                        </div>
                      </div>
                      <div className="flex items-center justify-end p-6 space-x-2 rounded-b border-t border-gray-200">
                        <button
                          // data-modal-toggle="actionModal"
                          type="button"
                          className="btn bg-white border-neutral-200 text-gray-500 hover:text-gray-700"
                          onClick={handleShowModal}
                        >
                          Cancel
                        </button>

                        {(permission?.isEdit || permission?.isDelete) &&
                          values.customerID === 0 && (
                            <button
                              disabled={GlobalLoading}
                              type="Submit"
                              className={`flex justify-center btn px-6 bg-indigo-500 hover:bg-indigo-600 text-white`}
                            >
                              <div
                                className={`w-full flex justify-center align-middle `}
                              >
                                Save
                              </div>
                            </button>
                          )}
                      </div>
                    </FormikForm>
                  );
                }}
              </Formik>
            </div>
          </div>
        </div>
      </div>
    </>
  );
};

export default CustomerAddUpdateModal;
