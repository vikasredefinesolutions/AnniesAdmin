  ...
  return (
    <>
      <div className="w-full relative">
        <div className="p-5">
          <div className="w-full flex flex-wrap sm:auto-cols-max justify-between gap-2 p-4">
            {displaySearch && (
              <TableSearch
                filter={globalFilter}
          ...
          <div className="overflow-x-scroll">
            <div className="table-auto w-full text-sm text-[#191919] font-semibold">
              <div className="thead text-sm font-bold uppercase text-[#b3b3b3] pt-2">
                <div className={`grid lg:grid-cols-${12}  md:grid-cols-${12} grid-cols-${12} border-t-2`}>
                  {columns.map((column, i) => {
                    return (
                      <div className={`flex flex-row px-2 py-4 ${column?.colSpan ? `col-span-${column?.colSpan}` : ""}`} key={i} >
                        {i === 0 && (
                          <div className="leading-none mr-2 w-6 h-6 cursor-pointer transition-all variant-arrow"
                        ...
                        <div className={`font-semibold text-left ${i === 0 ? "w-screen" : "w-52"} max-w-2xl flex items-center`}>
...
const NestedColumnDroppable = ({
  ...
  const handleDragEnd = (e) => {
    ...
    if (orderChangeHandler instanceof Function) {
      orderChangeHandler(e.destination.index, source_data, tempData);
    }
  };
...
const Column = ({
  ...
  return (
    <Fragment key={index}>
      <div {...provider.draggableProps} ref={provider.innerRef} className={`${snapshot.isDragging && "bg-[#f8fafc] outline-dashed"}`}>
        {ColumnComponent ? (
          <ColumnComponent
            provider={provider}
            columnValue={columnValue}
          ...
          <div
            className={`${isSubRow ? extraClass : `grid lg:grid-cols-${12} md:grid-cols-${12} grid-cols-${12}`} border-t border-neutral-200`}
          >
            {allColumns.map((column, i) => {
              return (
                <div
                  className={`${i === 0 ? `flex-auto row-start-auto` : ""} px-2 py-4 ${column?.colSpan ? `col-span-${column?.colSpan}` : ""}`}
                  key={i}
                >
                  <div
